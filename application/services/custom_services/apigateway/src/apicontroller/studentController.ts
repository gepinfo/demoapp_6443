...
this.router.get('/student/userid/created_by', this.GpGetNounCreatedBy);
        //#@gepdelimeterone@#
        //#@ssofacebookapiroute@#
        //#@ssogithubapiroute@#
        //#@gepbankingapiroute@#
...
public GpDelete(req: Request, res: Response) {
        ...
        new ApiAdapter().delete(Constant.STUDENTURL + `${req.url}` )
...
public GpUpdate(req: Request, res: Response) {
        ...
        new ApiAdapter().put(Constant.STUDENTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.STUDENTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
...
public GpGetNounCreatedBy(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into studentController.ts: GpGetNounCreatedBy');
        new ApiAdapter().get(Constant.STUDENTURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
            new CustomLogger().showLogger('info', 'Exit from studentController.ts: GpGetNounCreatedBy');
        }).catch(err => {
            res.send(err);
        });
    }
